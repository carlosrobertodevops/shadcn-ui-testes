...
function generateColorVariations(baseColor: string): ColorVariations {
  if (baseColor.startsWith("oklch")) {
    const lightColor = baseColor.replace(OKLCH_REGEX, (_match, values) => {
      ...
      const lightness = Math.min(Number.parseFloat(parts[0]) + LIGHT_ADJUSTMENT, MAX_LIGHTNESS);
      const chroma = Math.max(Number.parseFloat(parts[1]) - CHROMA_LIGHT_ADJUSTMENT, 0);
      const hue = parts[2];
    ...
    const lighterColor = baseColor.replace(OKLCH_REGEX, (_match, values) => {
      const parts = values.split(" ");
      const lightness = Math.min(Number.parseFloat(parts[0]) + LIGHTER_ADJUSTMENT, MAX_LIGHTNESS);
      const chroma = Math.max(Number.parseFloat(parts[1]) - CHROMA_LIGHTER_ADJUSTMENT, 0);
      const hue = parts[2];
      return `oklch(${lightness} ${chroma} ${hue})`;
    });
  ...
  const lightColor = baseColor.replace(")", ` / ${OPACITY_LIGHT})`).replace("oklch", "oklch");
  const lighterColor = baseColor.replace(")", ` / ${OPACITY_LIGHTER})`).replace("oklch", "oklch");

  return { lightColor, lighterColor };
}

export function generateColorPalette(primaryColor: string, secondaryColor: string): ColorPalette {
...
export function applyColorPalette(primaryColor: string, secondaryColor: string) {
  if (typeof document === "undefined") {
    return;
  }
  ...
  body.style.setProperty("--color-brand-secondary-lighter", palette.secondaryLighter);
...
export function resetColorPalette() {
  ...
  const computedSecondary = getComputedStyle(body).getPropertyValue("--color-brand-secondary").trim();
...
export function persistColorPalette(primaryColor: string, secondaryColor: string) {
